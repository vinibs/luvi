		...
		foreach ($rules as $field => $fieldrules) {
			...
			foreach ($rule as $singlerule) {
				...
				if ($singlerule == 'required' && empty($value)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve ser preenchido'; // PT-BR
				 	$arrayErrors[] = $message;
				...
				if ($singlerule ==  'int' && !preg_match('/^[0-9]+$/', $value)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve conter um número inteiro'; // PT-BR
					$arrayErrors[] = $message;
					$errorsInput[$field] .= $message . '<br>';
				}
				...
				if ($singlerule ==  'decimal' && !preg_match('/^[0-9\.\,]+$/', $value)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve conter um número decimal'; // PT-BR
				...
				if ($singlerule ==  'alpha' && !preg_match('/^[a-zA-ZÀ-úçÇ ]+$/', $value)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve conter apenas letras'; // PT-BR
					$arrayErrors[] = $message;
					$errorsInput[$field] .= $message . '<br>';
				}
				...
				if ($singlerule ==  'alphanum' && !preg_match('/^[a-zA-ZÀ-úçÇ0-9 ]+$/', $value)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve conter apenas letras e números'; // PT-BR
					$arrayErrors[] = $message;
					$errorsInput[$field] .= $message . '<br>';
				}
				...
				if ($singlerule ==  'email' && !filter_var($value, FILTER_VALIDATE_EMAIL)) {
					$errors = TRUE;
					$message = '"' . $field . '" deve conter um endereço de e-mail válido'; // PT-BR
					$arrayErrors[] = $message;
					$errorsInput[$field] .= $message . '<br>';
				}
				...
				if (sizeof($rulepart = explode(':', $singlerule)) > 1) {
					...
					if ($rulepart[0] == 'min' && strlen($value) < trim($rulepart[1])) {
						$errors = TRUE;
				 		$message = '"' . $field . '" deve conter ao menos ' . $rulepart[1]
				 			. ' characteres'; // PT-BR
			 			$arrayErrors[] = $message;
						$errorsInput[$field] .= $message . '<br>';
					...
					if ($rulepart[0] == 'max' && strlen($value) > trim($rulepart[1])) {
						$errors = TRUE;
				 		$message = '"' . $field . '" deve conter no máximo ' . $rulepart[1]
				 			. ' characteres'; // PT-BR
				 		$arrayErrors[] = $message;
						$errorsInput[$field] .= $message . '<br>';
			 		}
					...
					if ($rulepart[0] == 'equal' && $data[$field] != $data[$rulepart[1]]) {
						$errors = TRUE;
				 		$message = '"' . $field . '" deve ser igual ao campo "' 
				 			. $rulepart[1] . '"'; // PT-BR
				 		$arrayErrors[] = $message;
						$errorsInput[$field] .= $message . '<br>';
			 		}
